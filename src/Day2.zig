...
pub fn main() anyerror!void {
    ...
    var policies = std.ArrayList(Policy).init(&gpa.allocator);
    defer policies.deinit();

    var toks = std.mem.tokenize(input, " :-\n");
    while (toks.next()) |lower| {
        try policies.append(.{
            .lower = try std.fmt.parseUnsigned(u8, lower, 0),
            .upper = try std.fmt.parseUnsigned(u8, toks.next().?, 0),
            .char = toks.next().?[0],
            .password = toks.next().?,
        });
    }
    ...
    // Part One
    {
        ...
        for (policies.items) |policy| {
            var count: i32 = 0;
            for (policy.password) |char| {
                if (char == policy.char) count += 1;
            }
            if (count >= policy.lower and count <= policy.upper) valid += 1;
        }

        print("Part I - Valid Passwords: {d}\n", .{valid});
    }
    ...
    // Part Two
    {
        ...
        for (policies.items) |policy| {
            const p_char = policy.char;
            const first_char = policy.password[policy.lower - 1];
            const second_char = policy.password[policy.upper - 1];

            const at_first: bool = first_char == p_char and second_char != p_char;
            const at_second: bool = first_char != p_char and second_char == p_char;

            if ((at_first and !at_second) or (at_second and !at_first)) valid += 1;
        }

        print("Part II - Valid Passwords: {d}\n", .{valid});
    }
}
